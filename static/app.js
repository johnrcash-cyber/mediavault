...
async function loadJellyfinLibraries() {
  try {
    ...
    $("#jellyfinLastResult").textContent = data.last_result
      ? `${data.last_result.processed || 0} processed · ${data.last_result.added || 0} added · ${data.last_result.updated || 0} updated · ${data.last_result.restored || 0} restored · ${data.last_result.removed || 0} removed · ${data.last_result.skipped || 0} skipped · ${data.last_result.failed || 0} failed`
      : "No sync results yet.";
  } catch (error) { $("#jellyfinError").textContent = error.message; }
}
...
function renderJellyfinSyncStatus(result) {
  ...
  $("#jellyfinSyncStatus").innerHTML = `<p><strong>Sync complete</strong><span>${result.processed || 0} processed · ${result.added || 0} added · ${result.updated || 0} updated · ${result.restored || 0} restored · ${result.removed || 0} removed · ${result.skipped || 0} skipped · ${result.failed || 0} failed</span></p>${libraryLines}`;
  $("#jellyfinSyncStatus").hidden = false;
  $("#jellyfinLastSync").textContent = `Last sync: ${new Date(result.last_sync).toLocaleString()}`;
  $("#jellyfinLastResult").textContent = `${result.processed || 0} processed · ${result.added || 0} added · ${result.updated || 0} updated · ${result.restored || 0} restored · ${result.removed || 0} removed · ${result.skipped || 0} skipped · ${result.failed || 0} failed`;
}
...
$("#refreshLibraryAction").addEventListener("click", async () => {
  ...
  try {
    ...
    toast(`${result.processed || 0} processed · ${result.added || 0} added · ${result.updated || 0} updated · ${result.restored || 0} restored · ${result.removed || 0} removed · ${result.skipped || 0} skipped · ${result.failed || 0} failed`, 6000);
    await loadDashboard();
